...
Leg::Leg(string name,float L1,float L2,float L3);
{
 ...
 {
    m_fTheta0=jointPos[0];
    m_fTheta1=jointPos[1];
    m_fTheta2=jointPos[2];
 }
...
Matrix<float,3,1> CLeg::ForwardKinematics(int mode)
{
 ...
 */
Matrix<float,3,1> CLeg::InverseKinematics(Matrix<float, 4, 3> cmdpos)
{
        ...
        // jointCmdPos(legNum, 0) = atan2(factor_x * x, sqrt((m_fL1 + factor_0 * sin(jointCmdPos(legNum, 2)) * m_fL2) * (m_fL1 + factor_0 * sin(jointCmdPos(legNum, 2)) * m_fL2) + (cos(jointCmdPos(legNum, 2)) * m_fL2) * (cos(jointCmdPos(legNum, 2)) * m_fL2) - x * x)) + factor_0 * atan2(cos(jointCmdPos(legNum, 2)) * m_fL2, m_fL1 + factor_0 * m_fL2 * sin(jointCmdPos(legNum, 2)));
         ...
         jointCmdPos(1, 0) = atan2(factor_x * x, sqrt((m_fL1 + factor_0 * sin(jointCmdPos(legNum, 2)) * m_fL2) * (m_fL1 + factor_0 * sin(jointCmdPos(legNum, 2)) * m_fL2) + (cos(jointCmdPos(legNum, 2)) * m_fL2) * (cos(jointCmdPos(legNum, 2)) * m_fL2) - x * x)) + factor_0 * atan2(cos(jointCmdPos(legNum, 2)) * m_fL2, m_fL1 + factor_0 * m_fL2 * sin(jointCmdPos(legNum, 2)));
