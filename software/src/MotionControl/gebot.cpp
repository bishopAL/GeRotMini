...
void CGebot::SetPhase(float tP, float tFGP, Matrix<float, 4, 2> tFSP)
{
    ...
    {   
            vfTimeForSwing(legNum) = vvfTimeForSwingPhase[legNum][1]-vvfTimeForSwingPhase[legNum][0];
    }
...
void CGebot::NextStep()
{
    ...
    {   
        ...
        {
            if(fTimePresent > vvfTimeForSwingPhase[legNum][0] - fTimePeriod/2 && fTimePresent < vvfTimeForSwingPhase[legNum][1] - fTimePeriod/2 )
        ...
        {     
            ...
            {
                mfTargetCoMPosition(legNum, pos) += vfTargetCoMVelocity(pos) * fTimePeriod;
                mfTargetCoMPosture(legNum,pos) +=vfTargetCoMVelocity(pos+3)*fTimePeriod;
            }
            if(abs(fTimePresent - vvfTimeForSwingPhase[legNum][1]) < 1e-4)  // if on the start pos 
            {
            ...
            if(abs(fTimePresent - vvfTimeForSwingPhase[legNum][0]) < fTimePeriod + 1e-4)  // if on the end pos   
        ...
        {
            Matrix<float, 1, 3> swingPhaseVelocity = -(mfStancePhaseEndPos.row(legNum) - mfStancePhaseStartPos.row(legNum)) / vfTimeForSwing(legNum) ;
            float x, xh, m, n, k;
            //cout<<"legNum_"<<(int)legNum<<":"<<swingPhaseVelocity.array()<<"  ";
            if( ( mfTimePresentForSwing(legNum) - vfTimeForSwing(legNum) * TimeHeight ) < 1e-4 && mfTimePresentForSwing(legNum) > -1e-4 && swingPhaseVelocity( 0, 0) != 0)
            {            
            ...
            else if( mfTimePresentForSwing(legNum) - vfTimeForSwing(legNum) < 1e-4 && swingPhaseVelocity( 0, 0) != 0)
            {
                for(uint8_t pos=0; pos<2; pos++)
                    mfLegCmdPos(legNum, pos) += swingPhaseVelocity(pos) * fTimePeriod * (1 - swingVelFactor * TimeHeight) / (1 - TimeHeight); // vfTargetCoMVelocity
                mfLegCmdPos(legNum, 2) -= StepHeight / vfTimeForSwing(legNum) / (1 - TimeHeight) * fTimePeriod;
            }  

            if(swingPhaseVelocity( 0, 0) == 0)      //first step
            {
                if( ( mfTimePresentForSwing(legNum) - vfTimeForSwing(legNum)/2 ) < 1e-4 && mfTimePresentForSwing(legNum) > -1e-4)
                    mfLegCmdPos(legNum, 2) += StepHeight / vfTimeForSwing(legNum) * 2 * fTimePeriod;
                if( ( mfTimePresentForSwing(legNum) - vfTimeForSwing(legNum)/2 ) > -1e-4)
                    mfLegCmdPos(legNum, 2) -=  StepHeight / vfTimeForSwing(legNum) * 2 * fTimePeriod;
            }
        }
