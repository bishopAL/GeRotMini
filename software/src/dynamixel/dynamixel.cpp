 ...
 * 
 * @param baudrate set the target baudrate
 * 
 * @note after doing this, you have to reuse this class and redefine the baudrate as the new one
 */
void DxlAPI::setBaudRate(uint8_t baudrate)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result;
    for(int i=0; i<MOTOR_NUM; i++)
    {
        dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, ID[i], ADDR_BAUD_RATE, baudrate, &dxl_error);
        ...
        {
            printf("Dynamixel#%d has been successfully changed baudrate \n", ID[i]);
        }
    }
}
